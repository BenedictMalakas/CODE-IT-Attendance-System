...
# ---------------------------------------------------------------------------
# QR Generation (auto — kept for backward URL compat, no manual action needed)
# ---------------------------------------------------------------------------

@role_required('chairperson', 'vits')
@require_POST
def generate_qr_view(request, pk):
    """Legacy endpoint — QR is now auto-generated on approval."""
    student = get_object_or_404(Student, id=pk)
    if QRToken.objects.filter(student=student).exists():
        messages.info(request, f'QR code already exists for {student.name}.')
    else:
        messages.info(request, f'QR codes are now auto-generated when students are approved.')
    return redirect('admin_panel:students')


# ---------------------------------------------------------------------------
# Set Expected Students (stub from URL)
# ---------------------------------------------------------------------------

@admin_required
def set_expected_view(request, event_id):
    event = get_object_or_404(Event, id=event_id)
    # Placeholder — kept to avoid URL breakage
    messages.info(request, f'Expected students feature coming soon for "{event.name}".')
    return redirect('admin_panel:events')
